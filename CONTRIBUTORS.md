# Contributors

Thanks to everyone who has shaped Adrian. See the bottom for how to add yourself.

## Core team

- [Shahtaj Khalid](https://github.com/shahtajkhalid)
- [Yanny Christoforidis](https://github.com/yanny-sec)
- [Netan Mangal](https://github.com/netan-sa) 
- [Max Corbridge](https://github.com/max-c-sa)
- [Muhammad Faizan](https://github.com/Muhammad-at-sec)

## Community

(No community contributions yet. This is where your name goes when you contribute.)

## How to add yourself

Open a pull request adding your name to the relevant section, in alphabetical order within that section. A one-line entry is fine:

```
- Your Name (@your-github-handle) - one-line description of what you worked on
```

If your contribution is not code (docs, design, testing, an issue report we acted on, a bug repro), say so in the description. Anything that materially shaped the project is worth listing.